...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create test-page-f6d1
  fly apps create test-page-f6d1-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app test-page-f6d1
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app test-page-f6d1-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'test-page-f6d1-staging' does not match app name in config file 'test-page-f6d1'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name test-page-f6d1-db
  fly postgres attach --app test-page-f6d1 test-page-f6d1-db

  fly postgres create --name test-page-f6d1-staging-db
  fly postgres attach --app test-page-f6d1-staging test-page-f6d1-staging-db
  ```
